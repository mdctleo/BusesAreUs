...
import static ca.ubc.cs.cpsc210.translink.parsers.ArrivalsParser.parseArrivals;
...
public class ArrivalsListFragment extends ListFragment {
    ...
    private ArrayList<Arrival> getArrivalsForSelectedStop()  {
        ArrayList<Arrival> arrivalListHolder = new ArrayList<>();

        Stop selectedStop = StopManager.getInstance().getSelected();

        for(Arrival next : selectedStop)
        {arrivalListHolder.add(next);}

        return  arrivalListHolder;
    }
