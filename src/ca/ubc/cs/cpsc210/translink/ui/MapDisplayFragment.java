    ...
    private void plotRoutes() {
        ...
        if (s != null) {
            ...
            for (Route route : s.getRoutes()) {
                ...
                Polyline polylineHolder;

                for (RoutePattern routePattern : route.getPatterns()) {

                    polylineHolder = new Polyline(mapView.getContext());
                    List<GeoPoint> geoPoints = new LinkedList<>();

                    LatLon latLonHolder = routePattern.getPath().get(0);
                    for (LatLon latLon : routePattern.getPath()) {
                        Double lat = latLon.getLatitude();
                        Double lon = latLon.getLongitude();
                        GeoPoint geoPoint = new GeoPoint(lat, lon);
                        Double lat1 = latLonHolder.getLatitude();
                        Double lon1 = latLonHolder.getLongitude();
                        GeoPoint geoPoint1 = new GeoPoint(lat1,lon1);



                        if(rectangleIntersectsLine(northWest,southEast,latLon,latLonHolder)){
                        ...
                        if(!rectangleIntersectsLine(northWest,southEast,latLon,latLonHolder)){
                            ...
                            polylineHolder.setPoints(geoPoints);
                            polylineHolder.setWidth(getLineWidth(zoomLevel));
                            polylineHolder.setColor(busRouteLegendOverlay.getColor(route.getNumber()));
                            busRouteOverlays.add(polylineHolder);
                            geoPoints.clear();
                            polylineHolder = new Polyline(mapView.getContext());
...
//                + southeast.getLatitude() + " " + southeast.getLongitude());
    ...
    private void markStops() {
        ...
        Drawable closestStopIconDrawable = getResources().getDrawable(R.drawable.closest_stop_icon);
          ...
          for (Stop next : StopManager.getInstance()) {
              Double lat = next.getLocn().getLatitude();
              Double lon = next.getLocn().getLongitude();
              GeoPoint point = new GeoPoint(lat,lon);
              List<String> routeNoHolder = new LinkedList<String>();
              for(Route route : next.getRoutes()){
                      routeNoHolder.add(route.getNumber());
              }


            if (rectangleContainsPoint(northWest, southEast,next.getLocn())) {
                ...
                stopMarker.setTitle(next.getNumber()+ " " + next.getName());
                ...
                if(marker != null && nearestStopMarker != null) {
                    if (marker.getRelatedObject().equals(nearestStopMarker.getRelatedObject())) {
       ...
       {nearestStopMarker = new Marker(mapView);}
        ...
        if(nearest != null){
            for(Marker marker : stopClusterer.getItems()){
                ...
                for(Marker marker1 : stopClusterer.getItems()){
                if(marker1.getRelatedObject().equals(nearest)){
                   marker1.setIcon(closestStopIconDrawable);
    ...
    private void handleLocationChange(Location location) {
        LatLon latLonHolder = new LatLon(location.getLatitude(),location.getLongitude());
        Stop s = StopManager.getInstance().findNearestTo(latLonHolder);

        updateMarkerOfNearest(s);
        locationListener.onLocationChanged(s,latLonHolder);
